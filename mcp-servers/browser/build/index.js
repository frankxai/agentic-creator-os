...
server.registerTool(
  ...
  async ({ url, waitUntil = "networkidle" }) => {
    try {
      const page2 = await ensureBrowser();
...
server.registerTool(
  ...
  async ({ selector }) => {
    try {
      ...
      accessibilityTree = await page2.accessibility.snapshot();
      const title = await page2.title();
      return {
...
server.registerTool(
  "click",
  {
    ...
    inputSchema: {
      ...
      timeout: z.number().default(5e3).describe("Timeout in milliseconds")
    },
    annotations: {
      destructiveHint: false
    }
  },
  async ({ selector, timeout = 5e3 }) => {
    try {
      const page2 = await ensureBrowser();
      await page2.click(selector, { timeout });
      return {
...
server.registerTool(
  ...
  async ({ selector, value }) => {
    try {
      const page2 = await ensureBrowser();
      await page2.fill(selector, value);
      return {
...
server.registerTool(
  ...
  async ({ path }) => {
    try {
      ...
      if (path) {
        await __require("fs/promises").writeFile(path, screenshot);
        return {
...
server.registerTool(
  "wait_for_selector",
  {
    ...
    inputSchema: {
      ...
      timeout: z.number().default(1e4).describe("Timeout in milliseconds")
    },
    annotations: {
      readOnlyHint: true
    }
  },
  async ({ selector, timeout = 1e4 }) => {
    try {
      const page2 = await ensureBrowser();
      await page2.waitForSelector(selector, { timeout });
      return {
