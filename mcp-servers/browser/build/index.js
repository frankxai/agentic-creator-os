...
const timeout = (fallback) => z.number().int().min(100).max(120000).default(fallback);
function ok(data) {
...
server.registerTool("browser_navigate", {
    ...
    inputSchema: {
        ...
        timeout: timeout(30000).describe("Milliseconds to wait before failing")
    },
    outputSchema: {
...
server.registerTool("browser_get_page_content", {
    ...
    inputSchema: {
        ...
        maxChars: z.number().int().min(100).max(200000).default(20000).describe("Maximum characters of text to return")
    },
    outputSchema: {
...
server.registerTool("browser_fill_input", {
    title: "Fill input",
    ...
    inputSchema: {
        ...
        value: z.string().max(100000).describe("Text to put in the field")
    },
    outputSchema: {
...
server.registerTool("browser_screenshot", {
    ...
    try {
        ...
        if (path) {
            ...
            return ok({ path: target, format: "png", bytes: png.length });
        }
        return ok({ screenshot: png.toString("base64"), format: "png", bytes: png.length });
    }
    catch (error) {
...
server.registerTool("browser_evaluate", {
    ...
    inputSchema: {
        script: z.string().min(1).max(50000).describe("JavaScript expression or IIFE evaluated in the page; its result must be JSON-serialisable")
    },
    outputSchema: {
...
server.registerTool("browser_get_links", {
...
server.registerTool("browser_wait_for_selector", {
    ...
    inputSchema: {
        ...
        timeout: timeout(10000).describe("Milliseconds to wait before failing")
    },
    outputSchema: {
