...
const MEMORY_TYPES = ["observation", "reflection", "plan", "fact"] as const;
const sqlInput = z.string().min(1).max(20_000);
...
server.registerTool(
  "database_execute",
  {
    ...
    inputSchema: {
      ...
      params: z.array(z.string().max(100_000)).max(100).default([]).describe("Values bound to the ? placeholders in order, as text; SQLite column affinity converts numeric text for INTEGER and REAL columns")
    },
    outputSchema: {
...
server.registerTool(
  ...
  async ({ id }) => {
    try {
      ...
      return ok({ article: result.rows[0] ? toArticle(result.rows[0] as Row) : null });
    } catch (error) {
...
server.registerTool(
  ...
  async ({ status, limit }) => {
    try {
      ...
      const result = await db.execute({ sql, args: params });
      return ok({ articles: result.rows.map((row) => toArticle(row as Row)) });
    } catch (error) {
      return fail(`Error: ${error}`);
    }
  }
...
server.registerTool(
  ...
  async ({ name }) => {
    try {
      ...
      if (!row) return ok({ workflow: null });
...
server.registerTool(
  "database_save_workflow",
  {
    ...
    inputSchema: {
      ...
      config: z.string().max(50_000).optional().describe("Optional JSON object text with workflow settings, e.g. {\"cadence\":\"weekly\"}")
    },
    outputSchema: {
...
server.registerTool(
  ...
  async ({ key }) => {
    try {
      ...
      const row = result.rows[0] as Row | undefined;
      if (!row) return ok({ key, found: false });
      const type = String(row.type) as "string" | "number" | "json";
...
server.registerTool(
  "database_set_key_value",
  {
    ...
    inputSchema: {
      ...
      value: z.string().max(100_000).describe("Value as text; for type json pass JSON, for type number a numeric string"),
  ...
  async ({ key, value, type }) => {
    if (type === "number" && !Number.isFinite(Number(value))) return fail(`'${value}' is not a number; use type string or pass a numeric value`);
    if (type === "json") {
      try {
        JSON.parse(value);
      } catch {
...
server.registerTool(
  "database_store_memory",
  {
    ...
    inputSchema: {
      ...
      content: z.string().min(1).max(20_000).describe("Memory text"),
...
server.registerTool(
  ...
  async ({ agent_id, memory_type, limit }) => {
    try {
      ...
      const params: Array<string | number> = [agent_id];
      ...
      const result = await db.execute({ sql, args: params });
      const memories = result.rows.map((row) => toMemory(row as Row));
