...
var FROM = () => process.env.SMTP_FROM || "noreply@example.com";
var recipient = z.string().email().max(254);
var templateName = z.string().regex(/^[a-z][a-z0-9_-]{0,31}$/);
var sentOutput = {
...
var sendAnnotations = { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true };
function ok(data) {
...
function addresses(list) {
  return Array.isArray(list) ? list.map((entry) => typeof entry === "string" ? entry : String(entry.address ?? entry)) : [];
}
...
server.registerTool(
  "email_send_email",
  {
    ...
    inputSchema: {
      ...
      text: z.string().max(1e5).optional().describe("Plain-text body; send text, html or both"),
      html: z.string().max(2e5).optional().describe("HTML body"),
  ...
  "email_send_template_email",
  {
    ...
    inputSchema: {
      ...
      variables: z.array(z.object({
        ...
        value: z.string().max(5e3).describe("Text substituted for {{name}}")
...
server.registerTool(
  "email_send_notification",
  {
    ...
    inputSchema: {
      ...
      message: z.string().min(1).max(2e4).describe("Notification body text"),
...
server.registerTool(
  "email_add_template",
  {
    ...
    inputSchema: {
      ...
      content: z.string().min(1).max(2e4).describe("Template body with {{variable}} placeholders")
    },
    outputSchema: {
